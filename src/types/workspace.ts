...
export interface PlannerPlan { question: string; objective: string; location: string | null; geographicScope: string; requiredDatasets: Array<{ name: string; purpose: string; kind: string }>; constraints: Array<{ label: string; value: string; source: "user" | "planner" }>; assumptions: string[]; answer: string; desiredOutput: string; workflowSummary: string; confidence: number; clarificationQuestions: string[]; executionReadiness: "ready" | "needs_data" | "needs_clarification" | "unsupported"; graph: { nodes: GraphNode[]; edges: GraphEdge[] }; dsl: SpatialDslStep[]; }
...
export type MapLayerKind = "stations" | "coverage" | "gaps" | "candidates" | "generic" | "roads" | "waterways" | "bridges";
export interface MapLayer { id: string; name: string; kind: MapLayerKind; geojson: string; featureCount: number; outputPath: string; source?: "agent" | "import"; }
export interface MapFocus { displayName: string; lat: number; lon: number; south: number; north: number; west: number; east: number; }
export interface CameraTarget { lon: number; lat: number; height?: number; label?: string; }
export interface RankedCandidate { id: string; rank: number; lon: number; lat: number; score: number; rationale: string; factors: Record<string, number>; }
export interface AgentProgressStep { id: string; label: string; status: "pending" | "active" | "done" | "error"; }
