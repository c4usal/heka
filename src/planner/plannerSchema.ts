...
export const spatialOperationSchema = z.enum(["LoadDataset", "Buffer", "Overlay", "Intersect", "Route", "Coverage", "RasterMath", "Score", "Rank", "Visualize"]);
...
export const plannerOutputSchema = z.object({
  ...
  workflow: z.array(z.object({ operation: spatialOperationSchema, label: z.string().min(2), inputs: z.array(z.string()), rationale: z.string().min(3) })).min(1),
...
export const plannerJsonSchema = {
  ...
  schema: {
    ...
    required: ["objective", "location", "geographicScope", "requiredDatasets", "constraints", "assumptions", "desiredOutput", "workflowSummary", "workflow", "confidence", "clarificationQuestions"],
    properties: {
      ...
      workflow: { type: "array", items: { type: "object", additionalProperties: false, required: ["operation", "label", "inputs", "rationale"], properties: { operation: { type: "string", enum: ["LoadDataset", "Buffer", "Overlay", "Intersect", "Route", "Coverage", "RasterMath", "Score", "Rank", "Visualize"] }, label: { type: "string" }, inputs: { type: "array", items: { type: "string" } }, rationale: { type: "string" } } } },
      confidence: { type: "integer", minimum: 0, maximum: 100 }, clarificationQuestions: { type: "array", items: { type: "string" } }
    }
  }
} as const;
