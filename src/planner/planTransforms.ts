...
export function transformPlannerOutput(question: string, output: PlannerOutput): PlannerPlan {
  ...
  return { question, objective: output.objective, location: output.location, geographicScope: output.geographicScope, requiredDatasets: output.requiredDatasets, constraints: output.constraints, assumptions: output.assumptions, answer: output.answer, desiredOutput: output.desiredOutput, workflowSummary: output.workflowSummary, confidence: output.confidence, clarificationQuestions: output.clarificationQuestions, executionReadiness: output.executionReadiness, graph: { nodes, edges }, dsl };
}
