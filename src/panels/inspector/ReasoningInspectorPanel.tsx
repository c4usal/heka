...
import { discoverOpenStreetMapDataset, formatInvokeError, mapLayerKindForDataset } from "../../datasets/datasetResolver";
...
export function ReasoningInspectorPanel() {
  const planner = useWorkspaceStore((state) => state.planner); const execution = useWorkspaceStore((state) => state.execution); const datasetResolutions = useWorkspaceStore((state) => state.datasetResolutions); const addResolvedMapLayer = useWorkspaceStore((state) => state.addResolvedMapLayer); const setMapFocus = useWorkspaceStore((state) => state.setMapFocus); const plan = planner.plan;
  const [discovery, setDiscovery] = useState<Record<string, string>>({});
  const inspectSource = async (datasetName: string, kind: string) => {
    if (!plan) return;
    setDiscovery((current) => ({ ...current, [datasetName]: "Searching OpenStreetMap…" }));
    try {
      const result = await discoverOpenStreetMapDataset(datasetName, plan.geographicScope, kind);
      setMapFocus(result.place);
      addResolvedMapLayer({ id: `osm-${datasetName.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`, name: datasetName, kind: mapLayerKindForDataset(datasetName, kind), geojson: result.geojson, featureCount: result.featureCount, outputPath: result.outputPath });
      setDiscovery((current) => ({ ...current, [datasetName]: `${result.featureCount.toLocaleString()} features imported and added to the globe.` }));
    } catch (error) { setDiscovery((current) => ({ ...current, [datasetName]: formatInvokeError(error) })); }
  };
  return <aside className="inspector-panel"><header><span>REASONING INSPECTOR</span><strong>{planner.status === "planning" ? "Planning spatial workflow" : plan ? "Validated spatial plan" : "Workspace context"}</strong></header>{!plan ? <EmptyInspector /> : <>
    <section className="inspector-card"><div><FileText size={14} /><h2>Answer</h2></div><p>{plan.answer}</p><p className="detail"><MapPin size={11} /> {plan.location ?? plan.geographicScope}</p></section>
    <section className="inspector-card"><div><FileText size={14} /><h2>Intent</h2></div><p>{plan.objective}</p></section>
    ...
    <section className="inspector-card"><div><ListTree size={14} /><h2>Dataset resolver</h2></div>{datasetResolutions.map((resolution) => <p className="detail" key={resolution.id}><b>{resolution.datasetName}</b> · {resolution.status === "workspace" ? "Ready locally" : resolution.status === "discoverable" ? `Discoverable via ${resolution.sourceName}` : `Needs data from ${resolution.sourceName}`}<br /><small>{resolution.detail}</small>{resolution.status === "discoverable" && <><br /><button className="source-discovery" onClick={() => void inspectSource(resolution.datasetName, resolution.kind)}>Check source</button>{discovery[resolution.datasetName] && <small>{discovery[resolution.datasetName]}</small>}</>}</p>)}</section>
