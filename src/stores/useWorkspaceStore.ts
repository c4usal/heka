...
import type { Analysis, Dataset, ExecutionResult, ExecutionState, NavigationSection, PanelLayout, PlannerPlan, PlannerState, Project, RuntimeState, SelectedMapFeature, TimelineState, Workspace } from "../types/workspace";
...
interface WorkspaceStore { workspace: Workspace; datasets: Dataset[]; analyses: Analysis[]; timeline: TimelineState; planner: PlannerState; runtime: RuntimeState; execution: ExecutionState; setSection(section: NavigationSection): void; toggleSidebar(): void; setHorizontalLayout(layout: Pick<PanelLayout, "left" | "center" | "right">): void; setBottomSize(bottom: number): void; setTimelinePlaying(isPlaying: boolean): void; setRuntime(runtime: RuntimeState): void; beginPlanning(): void; applyPlan(plan: PlannerPlan): void; failPlanning(error: string): void; completeTimelineStep(index: number): void; beginExecution(): void; updateExecution(stage: string, progress: number, detail: string): void; completeExecution(result: ExecutionResult): void; selectMapFeature(feature?: SelectedMapFeature): void; failExecution(error: string): void; }
...
export const useWorkspaceStore = create<WorkspaceStore>()(persist((set) => ({
  ...
  beginExecution: () => set({ execution: { status: "running", progress: 0 } }), updateExecution: (stage, progress, detail) => set((state) => ({ execution: { ...state.execution, status: "running", stage, progress, detail } })), completeExecution: (result) => set({ execution: { status: "complete", progress: 100, result } }), selectMapFeature: (selectedFeature) => set((state) => ({ execution: { ...state.execution, selectedFeature } })), failExecution: (error) => set((state) => ({ execution: { ...state.execution, status: "error", error } }))
