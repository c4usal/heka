...
import type { Analysis, Dataset, NavigationSection, PanelLayout, PlannerPlan, PlannerState, Project, RuntimeState, TimelineState, Workspace } from "../types/workspace";
...
interface WorkspaceStore { workspace: Workspace; datasets: Dataset[]; analyses: Analysis[]; timeline: TimelineState; planner: PlannerState; runtime: RuntimeState; setSection(section: NavigationSection): void; toggleSidebar(): void; setHorizontalLayout(layout: Pick<PanelLayout, "left" | "center" | "right">): void; setBottomSize(bottom: number): void; setTimelinePlaying(isPlaying: boolean): void; beginPlanning(): void; applyPlan(plan: PlannerPlan): void; failPlanning(error: string): void; completeTimelineStep(index: number): void; }
...
export const useWorkspaceStore = create<WorkspaceStore>()(persist((set) => ({
  ...
  planner: { status: "ready", notes: ["Planner is ready for a spatial objective.", "No workflow has been generated."] }, runtime: { status: "ready", backend: "PyQGIS adapter (local)" },
